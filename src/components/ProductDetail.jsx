...
export default function ProductDetail() {
  ...
  useEffect(() => {
    getProducts().then((products) => {
      ...
      if (found) {
        setSimilarProducts(
          products.filter((p) => p.category === found.category && p.id !== found.id)
        );
      }
    });
  }, [id]);
  ...
  // ✅ تابع افزودن با چک لاگین
  const handleAddToCart = (p) => {
    if (!user) {
      setShowLoginModal(true); // مودال باز می‌شود
      return;
    }
    const newCart = { ...cart };
    ...
    <div className="product-detail-page container py-5">
      ...
      <div className="row mt-4">
        ...
        <div className="col-md-6">
          ...
          <button
            className="btn btn-card-fill mt-2"
          ...
          <div key={p.id} className="col-12 col-md-6 col-lg-4">
            <div className="card h-100 shadow-sm border-0">
              ...
              <div className="card-body p-2 d-flex flex-column">
                <Link
                  to={`/product/${p.id}`}
                  className="text-decoration-none text-dark mb-auto"
                >
